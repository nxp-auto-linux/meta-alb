#
# This file is mostly identical to meta-fsl-arm/image_types_fsl.bbclass.
...
UBOOT_NAME_SDCARD ?= "u-boot-${UBOOT_MACHINE_SDCARD}${UBOOT_REALSUFFIX_SDCARD}"
...
IMAGE_CMD_uboot.mxsboot-sdcard = "mxsboot sd ${DEPLOY_DIR_IMAGE}/${UBOOT_NAME_SDCARD} \
...
IMAGE_CMD_uboot.mxsboot-nand = "mxsboot ${MXSBOOT_NAND_ARGS} nand \
                                             ${DEPLOY_DIR_IMAGE}/${UBOOT_NAME_SDCARD} \
...
generate_imx_sdcard () {
        ...
        case "${IMAGE_BOOTLOADER}" in
                ...
                if [ -n "${SPL_BINARY}" ]; then
                        dd if=${DEPLOY_DIR_IMAGE}/${SPL_BINARY} of=${SDCARD} conv=notrunc seek=${UBOOT_BOOTSPACE_SEEK} bs=512
                        dd if=${DEPLOY_DIR_IMAGE}/${UBOOT_NAME_SDCARD} of=${SDCARD} conv=notrunc seek=$(expr ${UBOOT_BOOTSPACE_SEEK} \+ 136) bs=512
                else
                        dd if=${DEPLOY_DIR_IMAGE}/${UBOOT_NAME_SDCARD} of=${SDCARD} conv=notrunc seek=${UBOOT_BOOTSPACE_SEEK} bs=512
                fi
                ;;
                barebox)
