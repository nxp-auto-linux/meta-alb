...
waf_do_configure() {
	${WAF_BINDIR}/waf configure --prefix=${prefix} ${WAF_EXTRA_CONF} ${EXTRA_OECONF}
}

waf_do_compile()  {
	${WAF_BINDIR}/waf build ${@get_waf_parallel_make(d)}
}

waf_do_install() {
	${WAF_BINDIR}/waf install --destdir=${D}
}
